...
def configure_argparser(argparser_obj):
    ...
    argparser_obj.add_argument("--projections",
                               action="store",
                               type=file_type,
                               dest="input_file",
                               required=True,
...
def main():
    ...
    # Create draft board
    input_df = pd.read_excel(in_file) if ".xls" in in_file else pd.read_csv(in_file)
    ...
    logging.debug("{0} total players with projections".format(len(input_df)))
    input_df = input_df[input_df.position.isin(league_config["global"]["pos"])]
    logging.debug("{0} players remain after removing non-position players...".format(len(input_df)))
    ...
    logging.debug("{0} ADP players remain after removing non-position players...".format(len(adp_df)))
    ...
    # Add columns for draft selection
